...
def nDCG(sorted_items, pos_item, train_pos_item, k=5):
    ...
    for index in range(len(sorted_items)):
        if sorted_items[index] in train_pos_item:
            ...
            valid_num += 1
            if (
                sorted_items[index] in filter_item
                and sorted_items[index] not in recommended_items
            ):
...
def evaluate():
    ...
    user_pos_items, user_train_pos_items = get_user_pos_items(
    ...
    ndcg5 = np.mean(
        [
            nDCG(
                user_item_lists[index], user_pos_items[user], user_train_pos_items[user]
            )
