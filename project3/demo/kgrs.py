...
class Dataloader:
    def __init__(
        ...
        self.n_user = (
            max(list(set(self.train_pos[:, 0]) | set(self.train_neg[:, 0]))) + 1
        )
        self.n_item = (
            max(list(set(self.train_pos[:, 1]) | set(self.train_neg[:, 1]))) + 1
        )
        self._load_ratings()
    ...
    def _add_recsys_to_kg(self):
        ...
        for interaction in self.train_pos:
            self.kg.append(
                (interaction[0], self.rel_dict["feedback_recsys"], interaction[1])
            )
        for interaction in self.train_neg:
            self.known_neg_dict.append(
                (interaction[0], self.rel_dict["feedback_recsys"], interaction[1])
            )
    ...
    def get_training_batch(self):
        ...
        while (
            len(neg_data) < len(self.kg) * self.neg_rate
            and sample_failed_time < sample_failed_max
        ):
                ...
                for fact in self.kg:
                    ...
                    if random.random() > 0.5:
                        if fact[0] >= self.n_entity:
                            ...
                            while (
                                tail in hr_tail_set[(fact[0], fact[1])]
                                and sample_failed_time < sample_failed_max
                            ):
                        ...
                        if fact[0] >= self.n_entity:
                            head = random.randint(
                                self.n_entity, self.n_entity + self.n_user - 1
                            )
                            while (
                                head in rt_head_set[(fact[1], fact[2])]
                                and sample_failed_time < sample_failed_max
                            ):
                                sample_failed_time += 1
                                head = random.randint(
        ...
        pos_batches = np.array_split(
            pos_data, max(1, len(pos_data) // self.train_batch_size)
        )
        ...
        return [
            [pos_batches[index], neg_batches[index]]
            for index in range(len(pos_batches))
        ]
...
class TransE(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        self.device = (
            torch.device("cuda:{}".format(device_index))
            if device_index >= 0
        ...
        self.ent_embedding = torch.nn.Embedding(
            self.ent_num, self.dim, device=self.device
        )
        self.rel_embedding = torch.nn.Embedding(
            self.rel_num, self.dim, device=self.device
        )
    ...
    def forward(self, head, rel, tail) -> torch.Tensor:
        ...
        if self.l1:
            score = torch.sum(
                torch.abs(torch.subtract(torch.add(head_emb, rel_emb), tail_emb)),
                dim=-1,
                keepdim=True,
            )
        else:
            score = torch.sum(
                torch.square(torch.subtract(torch.add(head_emb, rel_emb), tail_emb)),
                dim=-1,
                keepdim=True,
            )
        return -score
    ...
    def optimize(self, pos, neg):
        ...
        neg_matrix = torch.t(
            torch.matmul(neg_score, torch.t(torch.ones_like(pos_score)))
        )
        loss = torch.mean(
            torch.clamp(
                torch.add(torch.subtract(neg_matrix, pos_matrix), self.margin), min=0
            )
        )
        return loss
    ...
    def top_k_eval(self, users: List[int], k: int = 5):
        ...
        for user in users:
            ...
            for index in score_ast:
                ...
                if (
                    user not in train_user_pos_item
                    or item_list[index] not in train_user_pos_item[user]
                ):
